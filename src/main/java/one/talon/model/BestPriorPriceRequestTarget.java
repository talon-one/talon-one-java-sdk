...
public class BestPriorPriceRequestTarget {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("targetType", "audienceID"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("targetType"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BestPriorPriceRequestTarget.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in BestPriorPriceRequestTarget is not found in the empty JSON string", BestPriorPriceRequestTarget.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BestPriorPriceRequestTarget.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `BestPriorPriceRequestTarget` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
